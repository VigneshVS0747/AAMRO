...
import { VAddress } from 'src/app/Models/crm/master/Vendor';
...
@Component({
  selector: 'app-vaddress-cuv-dialog',
  templateUrl: './vaddress-cuv-dialog.component.html',
  styleUrls: ['./vaddress-cuv-dialog.component.css', '../../../../ums/ums.styles.css']
})
export class VaddressCuvDialogComponent {
  ...
  constructor(private fb: FormBuilder,
    ...
    public dialogRef: MatDialogRef<VaddressCuvDialogComponent>) {
  ...
  AddtoAddressList() {
    ...
    if (this.AddressForm.valid) {
      ...
      //this.exists = this.potentialCustomerAddress.some(item => item.primaryAddress == primary);
      this.vAddress.forEach(x => {
      ...
      this.existAddress = this.vAddress.some(item => item.addressName == addressName);
      if (this.existAddress) {
        Swal.fire({
          icon: "info",
          title: "Info",
          text: " Address already exist!",
          showConfirmButton: true,
        });
  ...
  getAddresstype() {
    this.addressCrudService.GetAllActiveAddressType().subscribe((result: any) => {
