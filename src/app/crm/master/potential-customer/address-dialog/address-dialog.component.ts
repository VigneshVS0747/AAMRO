...
import { PotentialCustomerAddress } from '../potential-customer.model';
...
@Component({
  ...
  styleUrls: ['./address-dialog.component.css','../../../../ums/ums.styles.css']
})
export class AddressDialogComponent {
  ...
  date= new Date;
  editedContact: PotentialCustomerAddress[];
  viewMode: boolean;
  userId$: string;
  ...
  ngOnInit() {
    this.GetUserId();
    this.iniForm();
    this.potentialCustomerAddress=this.data.list;
  ...
  AddtoAddressList() {
    ...
    if (this.AddressForm.valid) {
      const AddressDetail: PotentialCustomerAddress = {
      ...
      this.potentialCustomerAddress.forEach(x => {
      ...
      this.existAddress = this.potentialCustomerAddress.some(item => item.addressName == addressName);
      ...
      if(this.potentialCustomerAddress.length==0)
        {
  ...
  //Dropdown
  //#region address type autocomplete
  getAddresstype() {
