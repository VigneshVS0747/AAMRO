...
import { DocumentChecklist } from '../customer.model';
...
@Component({
  selector: 'app-customer-doc-dialog',
  templateUrl: './customer-doc-dialog.component.html',
  styleUrls: ['./customer-doc-dialog.component.css', '../../../../ums/ums.styles.css'],
  providers: [
...
export class CustomerDocDialogComponent {
  userId$: string;
  viewMode: boolean;
  customerDocumetList: DocumentChecklist[] = [];
  ...
  ngOnInit() {
    ...
    this.initializeForm();
    this.EditMode();
    this.customerDocumetList = this.data.list;
  }
  GetUserId() {
    ...
    if (evt?.target?.files && evt.target.files.length > 0) {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        if (supportedFileTypes.includes(fileType)) {
          ...
          const existingIndex = this.customerDocumetList.findIndex(img => img.documentName.toLowerCase() === fileName);
          if (existingIndex !== -1) {
  ...
  //       return;
  //     }
  //     this.customerDocumetList.forEach(x => {
  ...
  AddtodocumentList() {
    ...
    if (!documentDetail.documentName && (documentDetail.isCollected || documentDetail.collectedDate)) {
      Swal.fire({
        icon: "info",
        title: "Info",
        text: "Please upload the document!",
      ...
      this.exists = this.customerDocumetList.some(x => x.documentTypeName === this.documentName);
      if (this.exists) {
        Swal.fire({
          icon: "info",
          title: "Info",
