...
@Component({
  selector: 'app-verify-otpdialog',
  templateUrl: './verify-otpdialog.component.html',
  styleUrls: ['./verify-otpdialog.component.css']
})
export class VerifyOtpdialogComponent implements OnInit {
  ...
  constructor(private service:ChangepasswordService, private dialog: MatDialog,
    public dialogRef: MatDialogRef<VerifyOtpdialogComponent>) { }
  ...
  this.service.Verifyotp(payload).subscribe({
    next: (res) => {
      if(res.message=="success"){
        ...
        const dialogRef = this.dialog.open(ForgetpasswordchangedialogComponent, {
