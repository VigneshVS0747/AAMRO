...
@Component({
  ...
  styleUrls: ['./road-status.component.css'],
  providers: [
...
export class RoadStatusComponent {
  userId$: any;
  pageSize = 10;
  skip = 0;
  JOAirStatusList:any[]=[];
  ...
  constructor(private router: Router, private store: Store<{ privileges: { privileges: any } }>, private regionService: RegionService,
    ...
    @Inject(MAT_DIALOG_DATA) public data: any, private jobtypeService: JobtypeserviceService, public RoadTransportStatusDailog: MatDialogRef<AddJobOrderComponent>,
  ) { 
  ...
  ngOnInit(): void {
    ...
    if(this.data?.list){
      this.JOAirStatusList = this.data?.list;
    }
    this.GetUserId();
  }
  ...
  save(){
    if(this.myFormGroup.invalid){
      ...
      this.JOAirStatusList = [payload, ...this.JOAirStatusList]
      this.myFormGroup.reset();
    }
  }

  close(){
    this.RoadTransportStatusDailog.close(this.JOAirStatusList)
  }
