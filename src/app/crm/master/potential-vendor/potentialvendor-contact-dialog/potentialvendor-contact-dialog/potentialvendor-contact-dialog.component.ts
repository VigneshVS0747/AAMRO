...
import { ContactTypeService } from 'src/app/crm/master/contact-type/contact-type.service';
import { ContactType } from 'src/app/Models/crm/master/ContactType';
import { Observable, map, startWith } from 'rxjs';
import { PotentialVendorContact } from 'src/app/crm/master/potential-vendor/potential-vendor.model';
...
@Component({
  selector: 'app-potentialvendor-contact-dialog',
  templateUrl: './potentialvendor-contact-dialog.component.html',
  styleUrls: ['./potentialvendor-contact-dialog.component.css', '../../../../../ums/ums.styles.css']
})
export class PotentialvendorContactDialogComponent {
  liveStatus = 1;
  date = new Date;
  ...
  potentialVendorContact: PotentialVendorContact[] = [];
  exists: boolean;
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<PotentialvendorContactDialogComponent>
  ) {
  ...
  ngOnInit() {
    ...
    this.EditMode();
    this.potentialVendorContact = this.data.list;
  ...
  AddtoContactList() {
    ...
    if (this.ContactForm.valid) {
      const contactDetail: PotentialVendorContact = {
      ...
      this.potentialVendorContact.forEach(x => {
      ...
      if (this.potentialVendorContact.length == 0) {
